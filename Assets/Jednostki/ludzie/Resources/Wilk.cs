        ...
        int iloscWilkow = -2;
            for (int x = -1; x < 2; x++)
            {
                for (int y = -1; y < 2; y++)
                {
                    ...
                    {
                        GameObject unit = Menu.kafelki[posX][posY].GetComponent<Pole>().postac;
                        Jednostka wilkMoze = null;
                        if(unit != null)
                        {
                            wilkMoze = unit.GetComponent<Jednostka>();
                        }

                        if (wilkMoze != null && wilkMoze.nazwa == "Wilk")
                        {
                            iloscWilkow+=2;
                        }
                    }
